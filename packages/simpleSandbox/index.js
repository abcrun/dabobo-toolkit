...
import enhancedElement from './enhancedElement.js';
import VM from './vm';
...
class SecureSandbox {
  ...
  createIframeSandbox() {
    ...
    this.vm = new VM(iframe, this.options.docScope);
  }
  ...
  // 安全的样式隔离
  scopedStyles(styles, sid = this.rootid) {
  ...
  // 异步加载和验证脚本
  loadScripts(scripts, sid = this.rootid) {
    ...
    while (list.length) {
      ...
      if (script.src) {
        fetch(script.src)
          .then((response) => {
            if (!response.ok) {
              throw new Error(`获取脚本代码失败: ${script.src}`);
            }
